...
| Prompt system     | `internal/prompt/` (template registry, composer, section builders) |
...
| Memory bridge     | `internal/memory/bridge/` (embedder factory, cross-task extractor) |
...
| Layered context   | `internal/memory/layered/` (L0/L1/L2 compression)      |
| WS client         | `clients/ws/`                                           |
| TUI               | `clients/tui/`                                          |
| Prompt templates  | `internal/prompt/catalog.go` → all prompt constants + `DefaultRegistry` |
...
- **Prompt system** — `internal/prompt/` centralizes all prompt templates in a `Registry` with named IDs. `Composer` assembles sections and logs a structured manifest (`slog.Debug`) at each composition. Section builders (`ToolSection`, `SessionSection`, etc.) are pure functions with zero internal dependencies.

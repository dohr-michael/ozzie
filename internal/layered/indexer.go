...
func buildSummarizePrompt(text string, targetTokens int) string {
	if targetTokens <= 150 {
		return fmt.Sprintf(
			"Summarize the following conversation excerpt in 1-2 sentences (max %d tokens). "+
				"Focus on the key topic and outcome.\n\n%s", targetTokens, text)
	}
	return fmt.Sprintf(
		"Produce a structured bullet-point summary of the following conversation excerpt (max %d tokens). "+
			"Include key topics, decisions, and outcomes.\n\n%s", targetTokens, text)
}

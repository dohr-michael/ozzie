...
// ConstraintGuard wraps a brain.Tool with argument-level validation.
...
func WrapConstraint(t brain.Tool, name string) brain.Tool {
	return &ConstraintGuard{inner: t, toolName: name}
}

// Info delegates to the inner tool.
func (g *ConstraintGuard) Info(ctx context.Context) (*brain.ToolInfo, error) {
	return g.inner.Info(ctx)
}

// Run validates arguments against constraints before delegating.
func (g *ConstraintGuard) Run(ctx context.Context, argumentsInJSON string) (string, error) {
	constraints := events.ToolConstraintsFromContext(ctx)
	if constraints == nil {
		return g.inner.Run(ctx, argumentsInJSON)
	}
...
var _ brain.Tool = (*ConstraintGuard)(nil)

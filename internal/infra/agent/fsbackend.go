...
type OzzieBackend struct {
	...
	perms               *conscience.ToolPermissions  // nil = no accept-all bypass
}
...
func NewOzzieBackend(bus events.EventBus, perms *conscience.ToolPermissions, opts ...OzzieBackendOption) *OzzieBackend {
...
func (b *OzzieBackend) validateReadPath(ctx context.Context, path string) error {
	...
	if real, err := conscience.EvalSymlinksExisting(absPath); err == nil {
		absPath = real
	}
	...
	for _, restricted := range b.readRestrictedPaths {
		...
		if conscience.IsUnder(absPath, clean) {
...
func (b *OzzieBackend) isInsideSandbox(ctx context.Context, absPath string) bool {
	if wd := events.WorkDirFromContext(ctx); wd != "" {
		...
		if conscience.IsUnder(absPath, cleanWD) {
			return true
		}
	}

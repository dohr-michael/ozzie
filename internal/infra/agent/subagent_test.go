...
func TestSubAgentInstructions_ContainsToolNames(t *testing.T) {
	tools := []string{"ls", "read_file", "write_file", "edit_file", "run_command"}
...
func TestAgentInstructions_ContainsMemorySection(t *testing.T) {
	keywords := []string{"### Memory", "store_memory", "auto"}
...
func TestNewSubAgentMiddleware_InjectsInstructions(t *testing.T) {
	mw := NewSubAgentMiddleware("", brain.TierLarge)
...
func TestNewSubAgentMiddleware_WithRuntimeInstruction(t *testing.T) {
	...
	mw := NewSubAgentMiddleware(runtime, brain.TierLarge)
...
func TestNewSubAgentMiddleware_TierSmall(t *testing.T) {
	mw := NewSubAgentMiddleware("", brain.TierSmall)
	if mw.AdditionalInstruction != prompt.SubAgentInstructionsCompact {
		t.Errorf("expected compact instructions for brain.TierSmall")
	}
}

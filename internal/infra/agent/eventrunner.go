...
func (er *EventRunner) processMessage(sessionID string, content string) {
	...
	compressed := false
	if er.layered != nil {
		// Convert to domain messages for layered context
		domainMsgs := make([]brain.Message, len(messages))
		for i, m := range messages {
			domainMsgs[i] = brain.Message{Role: string(m.Role), Content: m.Content}
		}
		// Convert raw history to domain messages for the layered indexer
		domainHistory := make([]brain.Message, len(history))
		for i, m := range history {
			domainHistory[i] = brain.Message{Role: m.Role, Content: m.Content, Ts: m.Ts}
		}
		if lmsgs, ar, layeredErr := er.layered.Apply(ctx, sessionID, domainMsgs, domainHistory); layeredErr == nil {
			// Convert back to Eino schema messages
			messages = make([]*schema.Message, len(lmsgs))
			for i, m := range lmsgs {
